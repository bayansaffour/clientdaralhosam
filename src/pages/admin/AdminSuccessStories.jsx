...
const AdminSuccessStories = () => {
  ...
  const handleRestore = async (id) => {
    try {
      const token = localStorage.getItem("adminToken");
      await axios.patch(
        `${import.meta.env.VITE_BACKEND_URL}/api/success/${id}/restore`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
      fetchStories();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingStory) {
        await axios.put(
          `${import.meta.env.VITE_BACKEND_URL}/api/success/${editingStory._id}`,
          storyData,
          { headers: { Authorization: `Bearer ${token}` } }
        );
      } else {
        await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/success`, storyData, {
          headers: { Authorization: `Bearer ${token}` },
        });
      }
      setViewMode(true);
  ...
  const handleSoftDelete = async (id) => {
    try {
      const token = localStorage.getItem("adminToken");
      await axios.patch(
        `${import.meta.env.VITE_BACKEND_URL}/api/success/${id}/soft-delete`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
      fetchStories();
  ...
  const handleDelete = async (id) => {
    try {
      const token = localStorage.getItem("adminToken");
      await axios.delete(`${import.meta.env.VITE_BACKEND_URL}/api/success/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchStories();
    } catch (err) {
