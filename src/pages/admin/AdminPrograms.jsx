...
const AdminPrograms = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [programsRes, categoriesRes] = await Promise.all([
        axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/programs?includeDeleted=true`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
        axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/programs/categories`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
      ]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingProgram) {
        await axios.put(
          `${import.meta.env.VITE_BACKEND_URL}/api/programs/${editingProgram._id}`,
          formData,
          { headers: { Authorization: `Bearer ${token}` } }
        );
      } else {
  ...
  const handleSoftDelete = async (id) => {
    try {
      const token = localStorage.getItem("adminToken");
      await axios.patch(
        `${import.meta.env.VITE_BACKEND_URL}/api/programs/${id}/soft-delete`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleRestore = async (id) => {
    try {
      const token = localStorage.getItem("adminToken");
      await axios.patch(
        `${import.meta.env.VITE_BACKEND_URL}/api/programs/${id}/restore`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
      fetchData(); // Refresh the list to update the UI
