  ...
  const dummyStories = [
    {
      _id: "1",
      name: "ماريا قعاونه  ",
      title: " تعمل في الفريق الإداري ",
      imageUrl: "/placeholder1",
      shortStory:
        " قدرت اعزز مهاراتي في التفكير الإستراتيجي وإدارة الأزمات مما ساعدتني مواجهة التحديات بفعالية وابتكار حلول عملية وتحقيق نتائج إيجابية. ",
      fullStory:
       "خلال تطوعي وعملي في دار الحسام للعمل الشبابي، طورت مهاراتي بشكل كبير في مجالات مثل العمل الإداري، والتنظيم، والتنسيق، وبناء الشراكات. التجربة علمتني كيف أفهم احتياجات المجتمع بشكل أفضل، وخلّتني أؤمن أكتر بأهمية العمل المجتمعي ودوره الحقيقي في صناعة التغيير." ,
      testimonial:
        "دار الحسام لم تقدم لي التدريب فحسب، بل منحتني الثقة والدعم لاكتشاف إمكاناتي الحقيقية. أنا فخورة بما حققته وأتطلع لأستثماره في حياتي الشخصية والمهنية وعكسه على مجتمعي. " 
    },
    {
      _id: "2",
      name: " محمد زهير",
      ...
      shortStory:
      "منذ انضمامي كمُتطوع في مؤسسة دار الحسام للعمل الشبابي، بدأت رحلتي في تطوير مهاراتي الشخصية والمهنية.",
      fullStory:
        "  خلال هذه التجربة، استطعت بناء شبكة علاقات واسعة ومتنوعة في مجالات مختلفة، مما فتح أمامي فرصًا جديدة للنمو والتعلم. تعلمت معنى المسؤولية تجاه المجتمع، وأدركت أهمية العمل الجماعي والعطاء . "
        ,
      testimonial:
      "، أفتخر بكل ما اكتسبته من خبرات وصفات قيادية ساعدتني في أن أكون شخصًا فاعلًا ومؤثرًا في مجتمعي."
    },
    {
      _id: "3",
      name: " راشد المشاقبة ",
      ...
      shortStory:
"تطوعت في دار الحسام بدافع الشغف لخدمة المجتمع، وكانت تجربة غنية بالتحديات والتعلم المستمر.",
      fullStory:
      "انطلقت رحلتي في التطوع من خلال مؤسسة دار الحسام للعمل الشبابي ، وكانت تجربة مميزة شكلت نقطة تحول في مسيرتي.دخلت المجال بشغف، وحرصت على الاستفادة من كل فرصة تدريبية وتعليمية، بدءًا من تنظيم الفعاليات وصولًا إلى تنمية مهارات القيادة والتواصل.دار الحسام كانت الداعم الأول والبيئة اللي ساعدتني أكتشف قدراتي، وأؤمن بنفسي، وأسهم بشكل فعّال في خدمة مجتمعي.",
      testimonial:
 ...
 useEffect(() => {
  ...
  const handleSubmitStory = async (e) => {
    ...
    try {
      ...
      await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/success`, {
  ...
  const StoryCard = ({ story }) => (
    ...
    >
      <img
        src={story.imageUrl && story.imageUrl.startsWith('http') ? story.imageUrl : `${import.meta.env.VITE_BACKEND_URL}/${story.imageUrl?.replace(/^\/+/,'')}`}
        alt={story.name}
